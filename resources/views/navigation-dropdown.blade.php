<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="flex-shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}">
                        ...
                        {{--                        <img class="w-43 h-16" src="/images/logo.jpg" alt="new_pack_logo">--}}
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @canany(['read-order', 'write-order'])
                        <x-jet-nav-link href="{{ route('orders') }}" :active="request()->routeIs('orders')">
                            {{ __('Orders') }}
                        </x-jet-nav-link>
                    @endcanany

                    @canany(['read-user', 'write-user'])
                        <x-jet-nav-link href="{{ route('users') }}" :active="request()->routeIs('users')">
                            {{ __('Users') }}
                        </x-jet-nav-link>
                    @endcanany

                    @canany(['read-client', 'write-client'])
                        <x-jet-nav-link href="{{ route('clients') }}" :active="request()->routeIs('clients')">
                            {{ __('Clients') }}
                        </x-jet-nav-link>
                    @endcanany

                    @canany(['read-product', 'write-product'])
                        <x-jet-nav-link href="{{ route('products') }}" :active="request()->routeIs('products')">
                            {{ __('Products') }}
                        </x-jet-nav-link>
                    @endcanany

                    @canany(['read-material', 'write-material'])
                        <x-jet-nav-link href="{{ route('raw-materials') }}"
                                        :active="request()->routeIs('raw-materials')">
                            {{ __('Raw Materials') }}
                        </x-jet-nav-link>
                    @endcanany
                </div>
                            ...
                            <ul>
                                @foreach(Auth::user()->notifications as $notification)
                                    @if($notification->read_at === null)
                                        <li class="text-gray-800 py-2 px-3">
                                            @if($notification->type === 'App\Notifications\OrderCreated')
                                                Order #{{ $notification->data['id'] }} was created.
                                            @endif
                                        </li>
                                    @else
                                        <li class="text-gray-400 py-2 px-3">
                                            @if($notification->type === 'App\Notifications\OrderCreated')
                                                Order #{{ $notification->data['id'] }} was created.
                                            @endif
                                        </li>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @canany(['read-material', 'write-material'])
                <x-jet-responsive-nav-link href="{{ route('raw-materials') }}"
                                           :active="request()->routeIs('raw-materials')">
                    {{ __('Raw Materials') }}
                </x-jet-responsive-nav-link>
            @endcanany
